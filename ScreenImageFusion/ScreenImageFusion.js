class ScreenImageFusion extends Screen {
  constructor(container) {
    super(container, "./js/screens/ScreenImageFusion/ScreenImageFusion.html");
  }
  ...
  showPage(breadcrumbElem) {
    var self = this;
    this.$("#part1").removeClass("hidden");
    this.$("#part2").addClass("hidden");
    ...
    this.$(".page").each(function () {
      if ($(this).hasClass(pageClass)) {
        ...
        if (pageClass == "page5") {
          ...
          // 						self.popScreen();
          // 						self.parentScreen.loadAnatomicalMapping();
          // 					})
          self.popScreen();
          self.parentScreen.loadAnatomicalMapping();
  ...
  playVideoThenShowPage(indicator, videoID, pageName) {
    $(indicator).addClass("hidden");
    var vid = this.$("#" + videoID)[0];
    $(vid).unbind("ended");
    $(vid).on("ended", () => {
      this.showPage($(".breadcrumb[page=" + pageName + "]")[0]);
    });
  ...
  setupVideoControls() {
    ...
    $("#videoControls .slider").on("mousemove", (event) => {
      if (this.mouseDown) {
        var x = event.originalEvent.offsetX;
        var percent = x / parseInt($(".slider").css("width"));
        this.lastVisibleVid.currentTime =
          percent * this.lastVisibleVid.duration;
        this.lastVisibleVid.pause();
      }
    });
